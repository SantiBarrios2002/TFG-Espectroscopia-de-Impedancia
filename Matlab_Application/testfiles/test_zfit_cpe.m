% Standalone Zfit Test Script - CPE Model
% CPE (Constant Phase Element) model with impedance plot, R² value, and std deviation
...
mat_file = 'HojaB.mat';  % Change this to test different files:
                                % '100_10k_470n.mat', '10_100_6.8u.mat',
                                % '10_1k_470u.mat', '1k_1M_100p.mat',
                                % '510_100k_10n.mat'

fprintf('=== Zfit Analysis - CPE Model ===\n');
fprintf('File: %s\n\n', mat_file);
...
n_guess = 0.85;               % CPE exponent (typically 0.5-1.0)

circuit_string = 's(R1,p(R1,E2))';  % Rs in series with R||CPE
param_guess = [Rs_guess, R_guess, Q_guess, n_guess];

fprintf('Circuit model: Rs + (R||CPE)\n');
fprintf('CPE impedance: Z_CPE = 1 / (Q*(jω)^n)\n');
fprintf('Fitting...\n');

% Fitting parameters
LB = [0, 0, 1e-12, 0.5];   % Lower bounds (n typically >= 0.5)
UB = [inf, inf, 1, 1.0];   % Upper bounds (n typically <= 1.0)
...
% Calculate characteristic frequency for CPE
% For CPE, use fc ≈ 1/(2π·R·Q)^(1/n) as approximation
fc_cpe = 1 / (2 * pi * pbest(2) * pbest(3))^(1/pbest(4));
fprintf('  fc ≈ %.2f Hz (approx)\n\n', fc_cpe);

fprintf('CPE Interpretation:\n');
if pbest(4) > 0.95
    fprintf('  n ≈ 1: CPE behaves like a near-ideal capacitor\n');
elseif pbest(4) > 0.75
    fprintf('  n = %.2f: Moderate dispersion, typical for bioimpedance\n', pbest(4));
elseif pbest(4) > 0.5
    fprintf('  n = %.2f: Significant dispersion/roughness effects\n', pbest(4));
else
    fprintf('  n = %.2f: Strong non-ideal behavior\n', pbest(4));
end
fprintf('\n');
